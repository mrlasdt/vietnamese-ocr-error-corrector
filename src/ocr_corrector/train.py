...
#local import
from .tool.common import alphabet
...
class Trainer(object):
    def __init__(self, model_type='seq2seq', device='cuda:1'):
        self.batch_size = 768
        ...
        if self.model_type == 'seq2seq':
            weight_path = './weights/seq2seq_1.pth'
            self.device = (self.device if torch.cuda.is_available() else "cpu")
            self.criterion = LabelSmoothingLoss(len(alphabet), 0).to(device)
    ...
    def create_dataset(self, test_ratio=0.2):
        # self.train_dataset = BasicDataset('dataloader/data/train_lmdb_6000000')
        # self.val_dataset = BasicDataset('./val_lmdb')

        # binhnews_dataset = BasicDataset('dataloader/data/train_lmdb_6000000')
        # wiki_dataset = BasicDataset('dataloader/data/train_lmdb_wiki')
        # full_dataset = torch.utils.data.ConcatDataset([binhnews_dataset, wiki_dataset])
        # full_dataset = BasicDataset('dataloader/data/train_lmdb_10000')
        # test_size = int(len(full_dataset) * test_ratio)
        # train_size = len(full_dataset) - int(len(full_dataset) * test_ratio)
        # train_dataset, test_dataset = torch.utils.data.random_split(full_dataset, [train_size, test_size])
        train_dataset = BasicDataset('dataloader/data/train_lmdb')
        test_dataset = BasicDataset('dataloader/data/val_lmdb')
    ...
    def create_dataloader(self, dataset, shuffle):
        ...
        data_loader = DataLoader(dataset, batch_size=self.batch_size, sampler=sampler,
                                 collate_fn=Collator(), shuffle=False, num_workers=8, pin_memory=True)

        # use for concat dataset
    ...
    def train(self):
        ...
        for _ in range(self.num_iters):
            ...
            if global_step % self.valid_every == 0:
                ...
                print("==============================================================================")
                info = "val_loss: {:.4f}, full_seq_acc: {:.4f}, word_acc: {:.4f}".format(
    ...
    def validate(self, fold_id):
        ...
        with torch.no_grad():
            pbar = tqdm(range(len(self.val_loader)), ncols=100,
...
Trainer().train()
acc_full_seq, acc_per_char = Trainer().precision(1)
