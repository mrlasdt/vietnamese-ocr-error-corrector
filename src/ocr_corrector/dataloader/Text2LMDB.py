from src.tool.utils import extract_phrases, gen_ngrams
import itertools
from tqdm import tqdm
from ocr_corrector.tool.common import NGRAMS
import re
import lmdb
...
# label_file = 'dataloader/data/corpus-full.txt'
...
for p in tqdm(range(val_data), desc='Creating val dataset ...'):
    if not re.match(char_regrex, p):
        continue

    for ngr in gen_ngrams(p, NGRAMS):
        if len(" ".join(ngr)) < NGRAMS * 7:
            ngram_text = " ".join(ngr)
