...
from src.tool.utils import get_bucket
import re
from ocr_corrector.tool.common import alphabet, chars_regrex, same_chars
import torch
import numpy as np
from ocr_corrector.tool.vocab import Vocab
import lmdb
import os
import json
import random
from ocr_corrector.dataloader.data_augmentation import random_replace_accent
...
class BasicDataset(Dataset):
    ...
    def build_cluster_indices(self):
        ...
        if not os.path.isfile(self.lmdb_path + '.json'):
            pbar = tqdm(
                range(self.__len__()),
            ...
            f = open(self.lmdb_path + '.json', "w")
            f.write(data)
            f.close()
        else:
            # Opening JSON file
            # f = open(os.path.basename(self.lmdb_path) + '.json')
            # f = open(os.path.basename(self.lmdb_path) + '.json', 'r')
            f = open(self.lmdb_path + '.json', 'r')
    ...
    def __getitem__(self, idx):
        ...
        #     # if rd_aug_idx < 3:
        #     # text = self.remove_random_accent(text, ratio=0.05)
        #     # elif rd_aug_idx < 6:
        #     # text = self.replace_accent_chars(text, ratio=0.2)
        #     # elif rd_aug_idx < 8:
        ...
        #     # else:
        #     # text = self.remove_random_space(ori)
        text = random_replace_accent(ori)
...
class Collator(object):
    def __call__(self, batch):
        ...
        for sample in batch:
            ...
            src = np.concatenate((text, np.zeros(MAXLEN - text_len, dtype=np.int32)))  # padding
        ...
        return torch.LongTensor(
            np.array(text_data)), torch.LongTensor(tgt_input), torch.LongTensor(tgt_output), torch.BoolTensor(
            tgt_padding_mask)


if __name__ == '__main__':
    data = BasicDataset('train_lmdb_10000')
